...
ALTER TABLE core.users ADD CONSTRAINT users_external_id_uniq UNIQUE (external_id);
ALTER TABLE core.users ADD CONSTRAINT users_client_id_name_uniq UNIQUE (client_id, name);

CREATE EXTENSION pgcrypto;

CREATE FUNCTION sp.update_user(_id BIGINT, _external_id TEXT, _name TEXT) RETURNS text AS $$
DECLARE
  _row RECORD;
BEGIN
  SELECT INTO _row * FROM core.users WHERE id = _id;
  IF FOUND THEN
    IF ROW(_row.external_id, _row.name) IS DISTINCT FROM ROW(_external_id, _name) THEN
      UPDATE core.users SET name = _name WHERE id  = _id;
      RETURN 'updated';
    ELSE
      RETURN 'unchanged';
    END IF;
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO core.users ( external_id, client_id, name, password, password_updated ) VALUES
    ( _external_id, _client_id, _name, crypt(_password, gen_salt('md5')), CURRENT_TIMESTAMP ) RETURNING id INTO _id;
    RETURN _id::TEXT;
  ELSE
...
CREATE FUNCTION sp.login(_external_id TEXT, _password TEXT) RETURNS text AS $$
DECLARE
  _row RECORD;
BEGIN
  SELECT INTO _row * FROM core.users WHERE external_id = _external_id;
  IF FOUND THEN
